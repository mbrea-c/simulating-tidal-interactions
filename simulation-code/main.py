...
class Galaxy:
    ...
    def __init__(self, mass, radius=25*kpc.value, rot=(0., 0.), n_rings=6, p_count=3000, \
    ...
    def generate(self, mode):
        if mode==0:
            ...
            total_r    = np.sum(np.linspace(self.lim_rings[0], self.lim_rings[1], self.n_rings))*self.radius
            ...
            for i in range(self.p_count):
                ...
                radius = self.lim_rings[1] * self.radius * ((random()*(1.0 - self.lim_rings[0] / self.lim_rings[1])) + self.lim_rings[0] / self.lim_rings[1])
...
class View:
    ...
    def __init__(self, galaxies, debug=False, timestep=0.01*T_mult, \
                 ...
                 view_mrg=25*kpc.value, view_ang=[0., 0.], matrix_m = False, n_frames=1, p_size=0.02, \
                 p_fill=True, p_mode=0, ctr_size=180):
    ...
    def plot(self):
        ...
        def __draw(self, galaxies, sc=self.sc):
            ...
            for gx in self.galaxies:
                ...
                self.ax3d.scatter(*gx.dat[0], marker="o", edgecolor=self.gc, facecolor=self.gc, s=self.ctr_size * gx.mass/M_mult, depthshade=False)
                
                if self.p_fill:
                    self.ax3d.scatter(*stars_pos, marker="o", s=self.p_size, edgecolor=self.sc, facecolor=self.sc, linewidths=4, depthshade=False, rasterized=True)
                else:
                    self.ax3d.scatter(*stars_pos, marker="o", s=self.p_size, edgecolor=self.sc, facecolor="None",  linewidths=4, depthshade=False, rasterized=True)
        ...
        self.fig.savefig(prefixes[self.p_mode] + "Fig4_%f_%d.pdf" % (fname, self.view_ang[0]), dpi=300)
